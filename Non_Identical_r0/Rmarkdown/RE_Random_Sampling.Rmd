...
colnames(Est_Parameter_RM)<-c("Subsample_Size",paste0("Theta",0:length(Simulated_Data$Theta[-1])))
...
colnames(Est_Bias_RM)<-c("Subsample_Size",paste0("Theta",0:length(Simulated_Data$Theta[-1])))
...
```{r Load Data AM}
Path<-here("Non_Identical_r0","Outputs",params$Model_Path,"RE_Random_Sampling","Assumed_Model/")

for (j in 1:length(Simulated_Data$All_Models[-1])) 
{
  load(here(Path,paste0("RE_Random_Sample_output_",j,".RData")))
  
  eval(parse(text=paste0("Est_Parameter_AM_",j,"<-as.data.frame(Est_Param_RE_RandomSample)")))
  eval(parse(text=paste0("Utility_AM_",j,"<-as.data.frame(Utility_RE_RandomSample)")))
  eval(parse(text=paste0("Est_Bias_AM_",j,"<-as.data.frame(Bias_RE_RandomSample)")))
  
  eval(parse(text=paste0("colnames(Est_Parameter_AM_",j,")<-c('Subsample_Size',paste0('Theta',0:length(Simulated_Data$Theta[-1])))")))
  eval(parse(text=paste0("colnames(Utility_AM_",j,")<-c('Subsample_Size',paste0(c('A','D'),'_optimality'))")))
  eval(parse(text=paste0("colnames(Est_Bias_AM_",j,")<-c('Subsample_Size',paste0('Theta',0:length(Simulated_Data$Theta[-1])))")))
}
...
```{r Final plots 1,fig.width=31,fig.height=7}
Plot_list<-list()
...
```{r Final plots 2,fig.width=31,fig.height=7}
Plot_list<-list()
