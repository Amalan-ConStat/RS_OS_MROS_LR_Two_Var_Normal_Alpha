...
```{r Load Data AM}
Path<-here("Non_Identical_r0","Outputs",params$Model_Path,"Random_Sampling","Assumed_Model/")

for (j in 1:length(Simulated_Data$All_Models[-1])) 
{
  load(here(Path,paste0("Random_Sample_output_",j,".RData")))
  
  eval(parse(text=paste0("Est_Parameter_AM_",j,"<-as.data.frame(Est_Param_RandomSample)")))
  eval(parse(text=paste0("Utility_AM_",j,"<-as.data.frame(Utility_RandomSample)")))
  eval(parse(text=paste0("Est_Bias_AM_",j,"<-as.data.frame(Bias_RandomSample)")))
...
```{r Final plots 1,fig.height=6,fig.width=31}
Plot_list<-list()
...
```{r Final plots 2,fig.height=6,fig.width=31}
Plot_list<-list()
